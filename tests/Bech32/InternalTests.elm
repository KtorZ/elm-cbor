...
import Bech32.Internal exposing (EncodingDirection(..), WordEncodingFailure(..), checksum, encodeWords, polymodStep)
import Expect
    ...
    describe "Bech32.Internal"
        ...
        , describe "encodeWords" <|
            let
                ...
                testEncodeWordsOk words5 words8 =
                    describe ("encodeWords\n\twords 5: " ++ Debug.toString words5 ++ "\n\twords 8: " ++ Debug.toString words8) <|
                        [ test "5 → 8" <|
                            \_ -> encodeWords Word5ToWord8 words5 |> Expect.equal (Ok words8)
                        , test "8 → 5" <|
                            \_ -> encodeWords Word8ToWord5 words8 |> Expect.equal (Ok words5)
                        ]
            in
            [ testEncodeWordsOk
            ...
            , test "encodeWords -> excess padding" <|
                \_ -> encodeWords Word5ToWord8 [ 14, 20, 15, 7, 13, 26, 0, 25, 18, 6, 11, 13, 8, 21, 4, 20, 3, 17, 2, 29, 3, 0 ] |> Expect.equal (Err ExcessPadding)
            , test "encodeWords -> non-zero padding" <|
                \_ -> encodeWords Word5ToWord8 [ 3, 1, 17, 17, 8, 15, 0, 20, 24, 20, 11, 6, 16, 1, 5, 29, 3, 4, 16, 3, 6, 21, 22, 26, 2, 13, 22, 9, 16, 21, 19, 24, 25, 21, 6, 18, 15, 8, 13, 24, 24, 24, 25, 9, 12, 1, 4, 16, 6, 9, 17, 1 ] |> Expect.equal (Err NonZeroPadding)
            ]
        ]
